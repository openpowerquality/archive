var iticPlotter = (function () {
  ...
  function addPoint(duration, percentNominalVoltage) {
    ...
    switch (getRegionOfPoint(duration, percentNominalVoltage)) {
      case Region.NO_INTERRUPTION:
        events = noInterruptionEvents.data;
        break;
      case Region.PROHIBITED:
        events = prohibitedEvents.data;
        break;
      case Region.NO_DAMAGE:
        events = noDamageEvents.data;
        break;
    }
  ...
  function getRegionOfPoint(duration, percentNominalVoltage) {
    if (isInProhibitedRegion(duration, percentNominalVoltage)) {
      return Region.PROHIBITED;
    }
    if (isInNoDamageRegion(duration, percentNominalVoltage)) {
      return Region.NO_DAMAGE;
    }
